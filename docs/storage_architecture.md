...
Sau một khoảng thời gian (mặc định mỗi **60 giây**, cấu hình qua `STORAGE_SYNC_INTERVAL_SECONDS`), tiến trình Background Worker chạy tác vụ `StorageRetrySyncService` để kiểm tra và đẩy lại các file đang tồn tại ở Local Outbox lên S3.
